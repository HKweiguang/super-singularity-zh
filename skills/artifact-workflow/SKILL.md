...
| 模式 | 触发条件 | 核心动作 |
|------|---------|---------|
| **新建模式** | 当前节点的输出不存在 | 加载框架级全局契约 + 当前节点的 type-top-level（如有）→ 解析上游 inputs → 按模板生成输出 → 经 artifact-review 审查 → 写入 |
| **修改模式** | 当前节点的输出已存在 | 加载现有输出 → 加载上游 → 扫描下游影响 → 执行修改 → 经 artifact-review 审查 → 更新 |
| **回改模式** | 执行中发现上游定义有缺陷/遗漏/矛盾 | 暂停 → 定位上游 → 评估下游影响 → 用户确认 → 级联修改 → 全量经 artifact-review 重新审查 |
...
询问用户是否级联修改/重执行下游节点
    ...
    ↓
重新验证（经 artifact-review 全量审查，禁止缩减维度）
```
...
└── .artifacts/
    ...
    └── review-rules.yaml         ← 可选：artifact-review 自定义检查清单规则（非 artifact-workflow 职责）
```
...
- **在审查节点输出时：**
  - **必需子技能：** 使用 `artifact-review` —— 对节点输出进行统一审查
- **在声称完成之前：**
